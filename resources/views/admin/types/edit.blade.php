    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.types.update", [$type->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="type">{{ trans('cruds.type.fields.type') }}</label>
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $type->type) }}" required>
                @if($errors->has('type'))
